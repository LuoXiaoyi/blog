...
3. 如果不是空树，则从根搜寻，如果值大于当前结点（*x*），但是当前结点有右孩子，则继续找当前结点的右孩子（*x=x->right*）。直到当前结点没有右孩子（*x->right==NULL*），新结点（*r*）作为当前结点的右孩子。
...
这个时候祖父节点G就是红色了。一般来说，祖父节G点是黑色，那么祖父节点G的父节点G->parent必须是红色节点，就又遇到这种情况。这个时候把指针指向祖父节点，然后再执行情况1。如果不满足这个条件了，就执行情况4。
...
其中上面一部分是新节点N是父节点P的右孩子，而P是其父节点的左孩子的情况，下面一部分是新结点N是父节点P的左孩子，而P是其父节点的右孩子的情况。
